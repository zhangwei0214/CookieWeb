...
public class CookieUtil {
	...
	public static void readCookieAndLogon(HttpServletRequest request,
		...
		//如果是login页面 则不去检查cookie
		String uri = request.getRequestURI();
		//  uri="/CookieWeb/"
		if(uri.endsWith("/") || uri.endsWith("/login.do") || uri.indexOf("login.jsp") >=0){
		...
		String cookieValueAfterDecode64 = new String(
				Base64.decodeBase64(cookieValue), "utf-8");
		...
		if (user != null) {
			...
			String md5ValueFromDB = getMD5(user.getUserName() + ":"
					+ user.getPassword()
			...
			if (md5ValueFromDB.equals(md5ValueInCookie)) {
